...
for i = 2:n + 1
    ...
    q(3) = (func(matrix(i-1,3) + q(2)*h/2, matrix(i-1,1) + h/2));
    q(4) = (func(matrix(i-1,3) + q(3)*h, matrix(i-1,1) + h));
    matrix(i,5) = (q(1) + 2*q(2) + 2*q(3) + q(4))/6;
    k(1) = matrix(i-1, 3);
    k(2) = (matrix(i-1, 3) + q(1)*h/2);
    k(3) = (matrix(i-1, 3) + q(2)*h/2);
    k(4) = (matrix(i-1, 3) + q(3)*h/2);
    matrix(i,4) = (k(1) + 2*k(2) + 2*k(3) + k(4))/6;
